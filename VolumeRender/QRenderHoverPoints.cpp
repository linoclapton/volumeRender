...
QRenderHoverPoints::QRenderHoverPoints(QWidget *widget, PointShape shape)
    : QWidget(widget)
{
    m_widget = widget;
	...
	connect(this, SIGNAL(oneDTFChanged()), m_widget, SLOT(OnHoverPointChanged()));
}


void QRenderHoverPoints::setEnabled(bool enabled)
{
...
bool QRenderHoverPoints::eventFilter(QObject *object, QEvent *event)
{
    ...
    case QEvent::MouseButtonPress:
        {
            ...
            if (me->button() == Qt::LeftButton) {
                ...
                if (index == -1) {
                    ...
                    m_colors.insert(pos, QColor(255, 255, 255, 200));
                    m_currentIndex = pos;
                    firePointChange();
...
void QRenderHoverPoints::paintPoints()
{
    QPainter p;
#ifdef QT_OPENGL_SUPPORT
...
static QPointF bound_point(const QPointF &point, const QRectF &bounds, int lock)
{
    ...
    qreal left = bounds.left() + 0.1 * (bounds.right() - bounds.left());
    qreal right = bounds.right() - 0.1 * (bounds.right() - bounds.left());
    qreal top = bounds.top() + 0.1 * (bounds.bottom() - bounds.top());
    qreal bottom = bounds.bottom() - 0.1 * (bounds.bottom() - bounds.top());

    if (p.x() < left || (lock & QRenderHoverPoints::LockToLeft)) p.setX(left);
    else if (p.x() > right || (lock & QRenderHoverPoints::LockToRight)) p.setX(right);

    if (p.y() < top || (lock & QRenderHoverPoints::LockToTop)) p.setY(top);
    else if (p.y() > bottom || (lock & QRenderHoverPoints::LockToBottom)) p.setY(bottom);

    return p;
}

void QRenderHoverPoints::setPoints(const QPolygonF &points)
{
...
void QRenderHoverPoints::movePoint(int index, const QPointF &point, bool emitUpdate)
{
...
void QRenderHoverPoints::firePointChange()
{
